...
本例程主要功能是连接豆包 volc rtc 云端并进行语音交互，可以适用于智能音箱产品、智能玩具、语音控制设备等。此示例是一个综合性较强的例程，使用了 ADF 提供的高封装简易实用接口。建议用户构建项目时，优先使用 ADF 提供的高封装接口，可快速简便地构建项目。

目前 demo 中支持两种模式， 一个是唤醒模式，一个是语音交互模式
- 唤醒模式是用户通过语音唤醒设备，唤醒后设备进入语音交互模式，用户可以与设备进行语音交互,menuconfig 可选用 ``
- 语音交互模式是用户可以直接与设备进行语音交互。，默认的是 语音交互模式， 即配置 'CONTINUOUS_CONVERSATION_MODE', 可修改。
> 注意：唤醒模式默认使用 `Hi 乐鑫` 去唤醒， 可以在  `menuconfig` 中修改唤醒词。
...
本例程支持的开发板在 `$ADF_PATH/examples/README_CN.md` 文档中[例程与乐鑫音频开发板的兼容性表格](../README_CN.md#例程与乐鑫音频开发板的兼容性)中有标注，表格中标有绿色复选框的开发板均可运行本例程。请记住，如下面的 [配置](#配置) 一节所述，可以在 `menuconfig` 中选择开发板, 目前只支持 `esp32s3` 芯片 。
...
│   └── byte_rtc               # 豆包 RTC 库
├── main
│   ├── audio_processor.c      # 音频处理， 包括录音，播放和唤醒检测初始化
│   ├── audio_processor.h  
│   ├── byte_rtc_request.c     # 豆包 RTC 请求 (默认不用的)
│   ├── byte_rtc_request.h
│   ├── config.h               # 配置文件
│   ├── main.c                 # 主程序
│   ├── volc_rtc.c             # 豆包 RTC 初始化，事件处理
│   └── volc_rtc.h
└── spiffs                     # 存放提示音文件
```
...
1. 本例程需要预先在注册[火山·引擎账号](https://www.volcengine.com/), 然后在[火山·引擎 开放平台](https://www.volcengine.com/docs/6348/1315561) 开通服务，申请一个临时 token，然后将 `AppId`, `AppKey`, `RoomId` 和 `UserId` 的拷贝到 'config.h'。

2. 在 `main.c` 中配置 wifi 的 SSID 和 PASSWORD。
...
- 例程开始运行后，开发板会先去尝试连接 Wi-Fi 网络，成功获取 IP 地址后，先进入房间, 当进入房间成功后，log 中会有 `volc_rtc: join room success` 的打印， 然后服务端会下发 "你好呀， 很高兴认识你， 哈哈哈"， 这时候就可以开始对讲了
```c
...
- 出现加入房间房间之后没有任何声音
 > 目前遇到的绝大多数是和 token 有关，请检查 token 的有效性
- 出现自问自答的现象
 > 需要确认 AEC 是否生效了, 现已先保存一下录音数据，可以将`ENABLE_AEC_DEBUG`打开，查看录音数据
